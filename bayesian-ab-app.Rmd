...
observeEvent(input$calculate_posterior, {
  ...
  # Generate posteriors using bayesian logistic regression and stanarm package
...
### How it works
We must accept that there may be many different approaches to A/B testing under the banner of Bayesian frameworks. Hence, your mileage on these generalizations may vary. A prior version of this app used the R package `stanarm` to create a logistic regression model which was used to produce posterior distributions from Markov Chain Monte Carlo (MCMC) simulations. We switched to a much simpler approach using baseR to run just Monte Carlo simulations for a beta distribution comprised of the test data + a weak `beta(1,1)` prior. The results were comparable in all our tests. While we can't hope to do justice to a full explanation of the methods, we can offer some brief orientation and send you off to some excellent resources for further reading.  
...
<!-- **Monte Carlo simulation** - Our method of Bayesian A/B testing uses Monte Carlo simulations to identify the posterior distribution. Essentially, the simulation creates thousands of permutations of the data (the control and test conversion rates) that are compatible with our observed data + our priors. This generates a distribution of potential values known as the *posterior distribution*.  -->
<!-- It's pretty cool! For more than you ever wanted to know about Bayesian inference and MCMC, checkout [this intro](https://nbviewer.org/github/CamDavidsonPilon/Probabilistic-Programming-and-Bayesian-Methods-for-Hackers/blob/master/Chapter3_MCMC/Ch3_IntroMCMC_PyMC3.ipynb#Algorithms-to-perform-MCMC){target="_blank"} and [this extensive article](https://towardsdatascience.com/bayesian-inference-problem-mcmc-and-variational-inference-25a8aa9bce29){target="_blank"}. -->
