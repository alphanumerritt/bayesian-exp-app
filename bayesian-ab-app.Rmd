...
# library(insight)
library(ggplot2)
# library(see) # can remove I think
...
![Bayes' Theorem. Source: Joseph Rocca, https://towardsdatascience.com/bayesian-inference-problem-mcmc-and-variational-inference-25a8aa9bce29](https://miro.medium.com/max/700/1*04pd7c6QIHXYHgAelzzWlg@2x.png){width=45%} [Image source: https://towardsdatascience.com/bayesian-inference-problem-mcmc-and-variational-inference-25a8aa9bce29](https://towardsdatascience.com/bayesian-inference-problem-mcmc-and-variational-inference-25a8aa9bce29){target="_blank"}

### How it works
We must accept that there may be many different approaches to A/B testing under the banner of Bayesian frameworks. Hence, your mileage on these generalizations may vary. This app uses the R package `stanarm` to create a logistic regression model which is used to produce posterior distributions from Markov Chain Monte Carlo (MCMC) simulations. While we can't hope to do justice to a full explanation of the methods, we can offer some brief orientation and send you off to some excellent resources for further reading.  
...
**MCMC** - Our method of Bayesian A/B testing uses Markov Chain Monte Carlo simulations to identify the posterior distribution. Essentially, the simulation creates thousands of permutations of the data (the control conversion rate & the test effect) and determines whether they could be from the same distribution as the test data. It makes a yes/no decision on each permutation and uses that to create boundaries for itself, ending up with a distribution of potential values known as the *posterior distribution*. It's pretty cool! For more than you ever wanted to know about Bayesian inference and MCMC, checkout [this intro](https://nbviewer.org/github/CamDavidsonPilon/Probabilistic-Programming-and-Bayesian-Methods-for-Hackers/blob/master/Chapter3_MCMC/Ch3_IntroMCMC_PyMC3.ipynb#Algorithms-to-perform-MCMC){target="_blank"} and [this extensive article](https://towardsdatascience.com/bayesian-inference-problem-mcmc-and-variational-inference-25a8aa9bce29){target="_blank"}.

**Posteriors** - The posterior distribution is comprised of many different values of the control conversion rate and the test effect that are compatible with the observed test data. We derive all of our inferences about the test by mainly doing very simple aggregations on this distribution. For example, to get the probability that B > A, we simply look at the proportion of samples in the posterior in which that condition is true. Pretty simple in the end!
...
### Visualize Range for Baseline Conversion Rate
...
### Visualize Potential Business Impact
...
Posterior samples will be calculated using Markov Chain Monte Carlo simulations with 5 chains at 11,000 iterations per chain with 1,000 warm-up samples dropped from each for a total of 50,000 samples in the final posterior distribution. This may seem like overkill but we've seen it improve stability of the posterior distribution.
...
> Note: it is still possible to observe slight variations in posterior distributions by recalculating results.
...
renderUI({
  if (!is.null(global_vars$posteriors)) {
    ...
    div(style="padding-left:30px",
      h5(style = paste0("color:",global_vars$color_key[1]),"Probability test is not inferior to control: ",paste0(round(global_vars$p_noninferior*100,1),"%")),
      h5("Probability test effect is negligible: ",paste0(round(global_vars$p_null*100,1),"%")),
      h5(style = paste0("color:",global_vars$color_key[2]),"Probability test is better than control: ",paste0(round(global_vars$p_better*100,1),"%")),
      h5(style = paste0("color:",global_vars$color_key[3]),"Probability test is non-negligible: ",paste0(round(global_vars$p_significant*100,1),"%")),
      h5(style = paste0("color:",global_vars$color_key[4]),"Probability test will deliver desired ROI: ",paste0(round(global_vars$p_roi*100,1),"%"))
    )
  } else {
...
Row {data-height=30}
-------------------------------------
**version 2.0** - To see version history, report bugs and submit feature requests [click here](https://github.com/alphanumerritt/bayesian-exp-app/issues){target="_blank"}.
